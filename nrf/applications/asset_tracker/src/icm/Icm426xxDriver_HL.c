...
int inv_icm426xx_device_reset(struct inv_icm426xx * s)
{
	 ...
	 */
	data = ICM426XX_CHIP_CONFIG_RESET_EN;
	status |= inv_icm426xx_write_reg(s, MPUREG_CHIP_CONFIG, 1, &data);
	if(status)
		return status;
	...
	do {
		...
		status |= inv_icm426xx_read_reg(s, MPUREG_OTP_SEC_STATUS_B1, 1, &data);
		status |= inv_icm426xx_set_reg_bank(s, 0);
		data &= BIT_OTP_SEC_STATUS_OTP_DONE;
	...
	} while (data != BIT_OTP_SEC_STATUS_OTP_DONE); 
	...
	/* Load hardware POR values in register cache */
	status |= inv_icm426xx_read_reg(s, MPUREG_INTF_CONFIG1, 4, &s->register_cache.intf_cfg_1_reg);
	status |= inv_icm426xx_read_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);
	...
	if(s->serif.serif_type == ICM426XX_UI_I3C){
		status |= s->serif.configure((struct inv_icm426xx_serif *)s);
	}
...
int inv_icm426xx_force_clock_source(struct inv_icm426xx * s, ICM426XX_INTF_CONFIG1_ACCEL_LP_CLK_t clk_src)
{
	if(clk_src == ICM426XX_INTF_CONFIG1_ACCEL_LP_CLK_WUOSC)
		s->register_cache.intf_cfg_1_reg &= (uint8_t)~BIT_ACCEL_LP_CLK_SEL_MASK;
	else
		s->register_cache.intf_cfg_1_reg |= (uint8_t)ICM426XX_INTF_CONFIG1_ACCEL_LP_CLK_RCOSC;

	return inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG1, 1, &s->register_cache.intf_cfg_1_reg);
}
...
int inv_icm426xx_enable_accel_low_power_mode(struct inv_icm426xx * s)
{
	...
	ICM426XX_PWR_MGMT_0_ACCEL_MODE_t accel_mode = (ICM426XX_PWR_MGMT_0_ACCEL_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_ACCEL_MODE_MASK);
	ICM426XX_PWR_MGMT_0_GYRO_MODE_t  gyro_mode = (ICM426XX_PWR_MGMT_0_GYRO_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_GYRO_MODE_MASK);
	...
	if ((accel_mode != ICM426XX_PWR_MGMT_0_ACCEL_MODE_LP) && 
		...
		acc_odr_bitfield = (ICM426XX_ACCEL_CONFIG0_ODR_t)(s->register_cache.accel_cfg_0_reg & BIT_ACCEL_CONFIG0_ODR_MASK);
	...
	s->register_cache.pwr_mngt_0_reg &= (uint8_t)~BIT_PWR_MGMT_0_ACCEL_MODE_MASK;
	s->register_cache.pwr_mngt_0_reg |= (uint8_t)ICM426XX_PWR_MGMT_0_ACCEL_MODE_LP;
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);
	...
	if (ICM426XX_PWR_MGMT_0_ACCEL_MODE_LP != accel_mode) {
		...
		if(s->wu_off_acc_odr_changes >= 8) {
			...
			cur_freq = (ICM426XX_ACCEL_CONFIG0_ODR_t)(s->register_cache.accel_cfg_0_reg & BIT_ACCEL_CONFIG0_ODR_MASK);
			...
			s->register_cache.accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_ODR_MASK;
			s->register_cache.accel_cfg_0_reg |= (uint8_t)new_freq;
			status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &s->register_cache.accel_cfg_0_reg);
			inv_icm426xx_sleep_us(200);
			s->register_cache.accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_ODR_MASK;
			s->register_cache.accel_cfg_0_reg |= (uint8_t)cur_freq;
			status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &s->register_cache.accel_cfg_0_reg);
		}
	}
...
int inv_icm426xx_enable_accel_low_noise_mode(struct inv_icm426xx * s)
{
	...
	s->register_cache.pwr_mngt_0_reg &= (uint8_t)~BIT_PWR_MGMT_0_ACCEL_MODE_MASK;
	s->register_cache.pwr_mngt_0_reg |= (uint8_t)ICM426XX_PWR_MGMT_0_ACCEL_MODE_LN;
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);
	inv_icm426xx_sleep_us(200);
...
int inv_icm426xx_disable_accel(struct inv_icm426xx * s)
{
	...
	gyro_mode = (ICM426XX_PWR_MGMT_0_GYRO_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_GYRO_MODE_MASK);
#if (!INV_ICM426XX_LIGHTWEIGHT_DRIVER)
	...
	s->register_cache.pwr_mngt_0_reg &= (uint8_t)~BIT_PWR_MGMT_0_ACCEL_MODE_MASK; //== ICM426XX_PWR_MGMT_0_ACCEL_MODE_OFF
	status = inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);
...
int inv_icm426xx_enable_gyro_low_noise_mode(struct inv_icm426xx * s)
{
	...
	s->register_cache.pwr_mngt_0_reg &= (uint8_t)~BIT_PWR_MGMT_0_GYRO_MODE_MASK;
	s->register_cache.pwr_mngt_0_reg |= (uint8_t)ICM426XX_PWR_MGMT_0_GYRO_MODE_LN;
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);
	inv_icm426xx_sleep_us(200);
...
int inv_icm426xx_disable_gyro(struct inv_icm426xx * s)
{
	...
	accel_mode = (ICM426XX_PWR_MGMT_0_ACCEL_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_ACCEL_MODE_MASK);
#if (!INV_ICM426XX_LIGHTWEIGHT_DRIVER)
	...
	s->register_cache.pwr_mngt_0_reg &= (uint8_t)~BIT_PWR_MGMT_0_GYRO_MODE_MASK; /* == ICM426XX_PWR_MGMT_0_GYRO_MODE_OFF */
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &s->register_cache.pwr_mngt_0_reg);
...
int inv_icm426xx_enable_fsync(struct inv_icm426xx * s)
{
	...
	/* Enable Fsync */
	s->register_cache.tmst_cfg_reg |= (uint8_t)ICM426XX_TMST_CONFIG_TMST_FSYNC_EN;
	status |= inv_icm426xx_write_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);
...
int inv_icm426xx_disable_fsync(struct inv_icm426xx * s)
{
	...
	/* Disable Fsync */
	s->register_cache.tmst_cfg_reg &= (uint8_t)~BIT_TMST_CONFIG_TMST_FSYNC_MASK; // == ICM426XX_TMST_CONFIG_TMST_FSYNC_DIS
	status |= inv_icm426xx_write_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);
...
int inv_icm426xx_configure_timestamp_resolution(struct inv_icm426xx * s, ICM426XX_TMST_CONFIG_RESOL_t resol)
{
	...
	s->register_cache.tmst_cfg_reg &= (uint8_t)~BIT_TMST_CONFIG_RESOL_MASK;
	s->register_cache.tmst_cfg_reg |= (uint8_t)resol;
	status |= inv_icm426xx_write_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);
...
int inv_icm426xx_get_data_from_registers(struct inv_icm426xx * s)
{
	...
	if(int_status & BIT_INT_STATUS_DRDY) {
		 ...
		 */
		if(s->serif.serif_type == ICM426XX_UI_I2C) {
			uint8_t data = 0;
...
int inv_icm426xx_get_data_from_fifo(struct inv_icm426xx * s)
{
	...
	if((int_status & BIT_INT_STATUS_FIFO_THS) || (int_status & BIT_INT_STATUS_FIFO_FULL)) {
		...
		status |= inv_icm426xx_read_reg(s, MPUREG_FIFO_BYTE_COUNT1, 2, data);
		...
		if (packet_count > 0) {
			...
			for(packet_count_i = 0; packet_count_i < packet_count; packet_count_i++) {
				...
				if ((header->bits.timestamp_bit) || (header->bits.fsync_bit)) {
					 ...
					 */
					if (header->bits.fsync_bit && (s->fsync_to_be_ignored == 0))
...
int inv_icm426xx_set_accel_frequency(struct inv_icm426xx * s, const ICM426XX_ACCEL_CONFIG0_ODR_t frequency)
{
	 ...
	 */
	ICM426XX_PWR_MGMT_0_ACCEL_MODE_t accel_pwr_mode = (ICM426XX_PWR_MGMT_0_ACCEL_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_ACCEL_MODE_MASK);
...
#endif
	s->register_cache.accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_ODR_MASK;
	s->register_cache.accel_cfg_0_reg |= (uint8_t)frequency;
	return inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &s->register_cache.accel_cfg_0_reg);
}

int inv_icm426xx_set_gyro_frequency(struct inv_icm426xx * s, const ICM426XX_GYRO_CONFIG0_ODR_t frequency)
{
	s->register_cache.gyro_cfg_0_reg &= (uint8_t)~BIT_GYRO_CONFIG0_ODR_MASK;
	s->register_cache.gyro_cfg_0_reg |= (uint8_t)frequency;
	return inv_icm426xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1, &s->register_cache.gyro_cfg_0_reg);
}

int inv_icm426xx_set_accel_fsr(struct inv_icm426xx * s, ICM426XX_ACCEL_CONFIG0_FS_SEL_t accel_fsr_g)
{
	s->register_cache.accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_FS_SEL_MASK;
	s->register_cache.accel_cfg_0_reg |= (uint8_t)accel_fsr_g;
	return inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &s->register_cache.accel_cfg_0_reg);
}

int inv_icm426xx_set_gyro_fsr(struct inv_icm426xx * s, ICM426XX_GYRO_CONFIG0_FS_SEL_t gyro_fsr_dps)
{
	s->register_cache.gyro_cfg_0_reg &= (uint8_t)~BIT_GYRO_CONFIG0_FS_SEL_MASK;
	s->register_cache.gyro_cfg_0_reg |= (uint8_t)gyro_fsr_dps;
	return inv_icm426xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1, &s->register_cache.gyro_cfg_0_reg);
}
...
int inv_icm426xx_get_accel_fsr(struct inv_icm426xx * s, ICM426XX_ACCEL_CONFIG0_FS_SEL_t * accel_fsr_g)
{
	if((s->fifo_highres_enabled) && (s->fifo_is_used == INV_ICM426XX_FIFO_ENABLED))
		...
		*accel_fsr_g = (ICM426XX_ACCEL_CONFIG0_FS_SEL_t)(s->register_cache.accel_cfg_0_reg & BIT_ACCEL_CONFIG0_FS_SEL_MASK);
...
int inv_icm426xx_get_gyro_fsr(struct inv_icm426xx * s, ICM426XX_GYRO_CONFIG0_FS_SEL_t * gyro_fsr_dps)
{
	if((s->fifo_highres_enabled) && (s->fifo_is_used == INV_ICM426XX_FIFO_ENABLED))
		...
		*gyro_fsr_dps = (ICM426XX_GYRO_CONFIG0_FS_SEL_t)(s->register_cache.gyro_cfg_0_reg & BIT_GYRO_CONFIG0_FS_SEL_MASK);
...
int inv_icm426xx_reset_fifo(struct inv_icm426xx * s)
{
	...
	gyr_mode = (ICM426XX_PWR_MGMT_0_GYRO_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_GYRO_MODE_MASK);
	acc_mode = (ICM426XX_PWR_MGMT_0_ACCEL_MODE_t)(s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_ACCEL_MODE_MASK);
	...
	if ( ( (gyr_mode != ICM426XX_PWR_MGMT_0_GYRO_MODE_OFF) || (acc_mode != ICM426XX_PWR_MGMT_0_ACCEL_MODE_OFF) )
		...
		/* In case no sensor is enabled or in accel low power mode, change the FIFO_MODE to â€œbypassâ€(00) mode to force the FIFO reset,
...
int inv_icm426xx_enable_timestamp_to_register(struct inv_icm426xx * s)
{
	...
	if(!s->tmst_to_reg_en_cnt) {
		 ...
		 */
		s->register_cache.tmst_cfg_reg |= (uint8_t)ICM426XX_TMST_CONFIG_TMST_TO_REGS_EN;
		status |= inv_icm426xx_write_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);
...
int inv_icm426xx_disable_timestamp_to_register(struct inv_icm426xx * s)
{
	...
	if(!s->tmst_to_reg_en_cnt) {
		...
		s->register_cache.tmst_cfg_reg &= (uint8_t)~BIT_TMST_CONFIG_TMST_TO_REGS_EN_MASK; /* == ICM426XX_TMST_CONFIG_TMST_TO_REGS_DIS */
		status |= inv_icm426xx_write_reg(s, MPUREG_TMST_CONFIG, 1, &s->register_cache.tmst_cfg_reg);
	}
	s->tmst_to_reg_en_cnt --;
...
/* CLKIN is not supported for all parts, prevent the CLKIN to be enabled in this case. */
		...
		status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG5_B1, 1, &data);
		status |= inv_icm426xx_set_reg_bank(s, 0);
		s->register_cache.intf_cfg_1_reg |= (uint8_t)ICM426XX_INTF_CONFIG1_RTC_MODE_EN;
		status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG1, 1, &s->register_cache.intf_cfg_1_reg);
		...
		s->register_cache.intf_cfg_1_reg &= (uint8_t)~BIT_RTC_MODE_MASK; /* == ICM426XX_INTF_CONFIG1_RTC_MODE_DIS */
		status |= inv_icm426xx_write_reg(s, MPUREG_INTF_CONFIG1, 1, &s->register_cache.intf_cfg_1_reg);
	}
...
{
	if ((s->register_cache.intf_cfg_1_reg & BIT_RTC_MODE_MASK) == ICM426XX_INTF_CONFIG1_RTC_MODE_EN)
		return 1;
	else
		return 0;
}
	...
	switch (fifo_config) {
		...
		case INV_ICM426XX_FIFO_ENABLED :
			...
			*/
			...
			data = (uint8_t)ICM426XX_FIFO_CONFIG_MODE_SNAPSHOT;
			status |= inv_icm426xx_write_reg(s, MPUREG_FIFO_CONFIG, 1, &data);
			s->register_cache.tmst_cfg_reg |= ICM426XX_TMST_CONFIG_TMST_EN;
...
{
	ICM426XX_TMST_CONFIG_RESOL_t tmst_resol = (ICM426XX_TMST_CONFIG_RESOL_t)(s->register_cache.tmst_cfg_reg & BIT_TMST_CONFIG_RESOL_MASK);
...
static int inv_icm426xx_init_hardware_from_ui(struct inv_icm426xx * s)
{
	...
	s->register_cache.gyro_cfg_0_reg &= (uint8_t)~BIT_GYRO_CONFIG0_FS_SEL_MASK;
	s->register_cache.gyro_cfg_0_reg |= (uint8_t)ICM426XX_GYRO_CONFIG0_FS_SEL_2000dps;
	s->register_cache.accel_cfg_0_reg &= (uint8_t)~BIT_ACCEL_CONFIG0_FS_SEL_MASK;
	s->register_cache.accel_cfg_0_reg |= (uint8_t)ICM426XX_ACCEL_CONFIG0_FS_SEL_4g;
	status |= inv_icm426xx_write_reg(s, MPUREG_GYRO_CONFIG0, 1, &s->register_cache.gyro_cfg_0_reg);
	status |= inv_icm426xx_write_reg(s, MPUREG_ACCEL_CONFIG0, 1, &s->register_cache.accel_cfg_0_reg);
	...
	s->register_cache.tmst_cfg_reg &= (uint8_t)~BIT_TMST_CONFIG_TMST_FSYNC_MASK; // == ICM426XX_FSYNC_CONFIG_UI_SEL_NO
	...
	status |= inv_icm426xx_configure_smd_wom(s, 
...
static int inv_icm426xx_is_wu_osc_active(struct inv_icm426xx * s)
{
	...
	gyr_mode = (ICM426XX_PWR_MGMT_0_GYRO_MODE_t)      (s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_GYRO_MODE_MASK);
	acc_mode = (ICM426XX_PWR_MGMT_0_ACCEL_MODE_t)     (s->register_cache.pwr_mngt_0_reg & BIT_PWR_MGMT_0_ACCEL_MODE_MASK);
	lp_clk   = (ICM426XX_INTF_CONFIG1_ACCEL_LP_CLK_t) (s->register_cache.intf_cfg_1_reg & BIT_ACCEL_LP_CLK_SEL_MASK);
